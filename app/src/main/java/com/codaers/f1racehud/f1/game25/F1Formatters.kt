package com.codaers.f1racehud.f1.game25

import com.codaers.f1racehud.f1.F1Metadata

object F1Formatters {
    fun formatMotion(p: PacketMotionData) = formatMotion25(p)
    fun formatSession(p: PacketSessionData) = formatSession25(p)
    fun formatLapData(p: PacketLapData) = formatLapData25(p)
    fun formatEvent(p: PacketEventData) = formatEvent25(p)
    fun formatParticipants(p: PacketParticipantsData) = formatParticipants25(p)
    fun formatCarSetups(p: PacketCarSetupData) = formatCarSetups25(p)
    fun formatCarTelemetry(p: PacketCarTelemetryData) = formatCarTelemetry25(p)
    fun formatCarStatus(p: PacketCarStatusData) = formatCarStatus25(p)
    fun formatFinalClassification(p: PacketFinalClassificationData) = formatFinalClassification25(p)
    fun formatLobbyInfo(p: PacketLobbyInfoData) = formatLobbyInfo25(p)
    fun formatCarDamage(p: PacketCarDamageData) = formatCarDamage25(p)
    fun formatSessionHistory(p: PacketSessionHistoryData) = formatSessionHistory25(p)
    fun formatTyreSets(p: PacketTyreSetsData) = formatTyreSets25(p)
    fun formatMotionEx(p: PacketMotionExData) = formatMotionEx25(p)
    fun formatTimeTrial(p: PacketTimeTrialData) = formatTimeTrial25(p)
    fun formatLapPositions(p: PacketLapPositionsData) = formatLapPositions25(p)
}

// Motion Data (ID 0)
fun formatMotion25(packet: PacketMotionData): String {
...
// Session Data (ID 1)
fun formatSession25(packet: PacketSessionData): String {
...
// Lap Data (ID 2)
fun formatLapData25(packet: PacketLapData): String {
...
// Event Data (ID 3)
fun formatEvent25(packet: PacketEventData): String {
...
// Participants (ID 4)
fun formatParticipants25(packet: PacketParticipantsData): String {
...
// Car Setups (ID 5)
fun formatCarSetups25(packet: PacketCarSetupData): String {
    val playerIdx = packet.header.playerCarIndex.toInt()
    ...
    return """
        ...
        Brakes:
          ...
          Engine Braking: ${setup.engineBraking}%        // ✅ NEW IN 25
        Tyre Pressures:
        ...
        Misc:
          ...
          Next Pitstop Front Wing: ${packet.nextFrontWingValue}   // ✅ NEW IN 25
    """.trimIndent()
}

// Car Telemetry (ID 6)
fun formatCarTelemetry25(packet: PacketCarTelemetryData): String {
    val playerIdx = packet.header.playerCarIndex.toInt()
...
// Car Status (ID 7)
fun formatCarStatus25(packet: PacketCarStatusData): String {
    val playerIdx = packet.header.playerCarIndex.toInt()
...
fun formatFinalClassification25(packet: PacketFinalClassificationData): String {
    val playerIdx = packet.header.playerCarIndex.toInt()
    ...
    return """
        ...
        Result Reason: ${car.resultReason}
...
// Lobby Info (ID 9)
fun formatLobbyInfo25(packet: PacketLobbyInfoData): String {
...
// Car Damage (ID 10)
fun formatCarDamage25(packet: PacketCarDamageData): String {
    val playerIdx = packet.header.playerCarIndex.toInt()
    ...
    return """
        ...
        Tyre Blisters: FL=${car.tyreBlisters[2]}%, FR=${car.tyreBlisters[3]}%, RL=${car.tyreBlisters[0]}%, RR=${car.tyreBlisters[1]}%
...
fun formatSessionHistory25(packet: PacketSessionHistoryData): String {
...
// Tyre Sets (ID 12)
fun formatTyreSets25(packet: PacketTyreSetsData): String {
...
// MotionEx (ID ?)
fun formatMotionEx25(packet: PacketMotionExData): String {
    return """
        MotionEx →
        ...
        WheelCamber FL=${packet.wheelCamber[2]}, FR=${packet.wheelCamber[3]}, RL=${packet.wheelCamber[0]}, RR=${packet.wheelCamber[1]}
        WheelCamberGain FL=${packet.wheelCamberGain[2]}, FR=${packet.wheelCamberGain[3]}, RL=${packet.wheelCamberGain[0]}, RR=${packet.wheelCamberGain[1]}
    """.trimIndent()
}

fun formatTimeTrial25(packet: PacketTimeTrialData): String {
...
fun formatLapPositions25(packet: PacketLapPositionsData): String {
    return buildString {
        appendLine("Lap Positions →")
        appendLine("NumLaps=${packet.numLaps}, LapStart=${packet.lapStart}")

        packet.positionForVehicleIdx.forEachIndexed { lap, row ->
            appendLine("Lap $lap: ${row.joinToString()}")
        }
    }
}