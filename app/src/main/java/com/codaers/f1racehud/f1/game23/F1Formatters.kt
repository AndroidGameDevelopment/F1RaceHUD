...
object F1Formatters {
    fun formatMotion(p: PacketMotionData) = formatMotion23(p)
    fun formatSession(p: PacketSessionData) = formatSession23(p)
    fun formatLapData(p: PacketLapData) = formatLapData23(p)
    fun formatEvent(p: PacketEventData) = formatEvent23(p)
    fun formatParticipants(p: PacketParticipantsData) = formatParticipants23(p)
    fun formatCarSetups(p: PacketCarSetupData) = formatCarSetups23(p)
    fun formatCarTelemetry(p: PacketCarTelemetryData) = formatCarTelemetry23(p)
    fun formatCarStatus(p: PacketCarStatusData) = formatCarStatus23(p)
    fun formatFinalClassification(p: PacketFinalClassificationData) = formatFinalClassification23(p)
    fun formatLobbyInfo(p: PacketLobbyInfoData) = formatLobbyInfo23(p)
    fun formatCarDamage(p: PacketCarDamageData) = formatCarDamage23(p)
    fun formatSessionHistory(p: PacketSessionHistoryData) = formatSessionHistory23(p)
    fun formatTyreSets(p: PacketTyreSetsData) = formatTyreSets23(p)
    fun formatMotionEx(p: PacketMotionExData) = formatMotionEx23(p)
}


// Motion Data (ID 0)
fun formatMotion23(packet: PacketMotionData): String {
...
// Session Data (ID 1)
fun formatSession23(packet: PacketSessionData): String {
...
// Lap Data (ID 2)
fun formatLapData23(packet: PacketLapData): String {
    val car0 = packet.lapData[packet.header.playerCarIndex.toInt()]

    // Sector 3 is derived: lastLapTime - (sector1 + sector2)
    val sector3Time = car0.lastLapTimeInMS.toInt() -
            (car0.sector1TimeInMS.toInt() + car0.sector2TimeInMS.toInt())
    ...
    return """
        ...
        Sector 1: ${car0.sector1TimeInMS} ms
        Sector 2: ${car0.sector2TimeInMS} ms
...
// Event Data (ID 3)
fun formatEvent23(packet: PacketEventData): String {
...
// Participants (ID 4)
fun formatParticipants23(packet: PacketParticipantsData): String {
...
// Car Setups (ID 5)
fun formatCarSetups23(packet: PacketCarSetupData): String {
    val playerIdx = packet.header.playerCarIndex.toInt()
...
// Car Telemetry (ID 6)
fun formatCarTelemetry23(packet: PacketCarTelemetryData): String {
    val playerIdx = packet.header.playerCarIndex.toInt()
...
// Car Status (ID 7)
fun formatCarStatus23(packet: PacketCarStatusData): String {
    val playerIdx = packet.header.playerCarIndex.toInt()
...
fun formatFinalClassification23(packet: PacketFinalClassificationData): String {
    val playerIdx = packet.header.playerCarIndex.toInt()
...
// Lobby Info (ID 9)
fun formatLobbyInfo23(packet: PacketLobbyInfoData): String {
    val headerInfo = "Players in Lobby: ${packet.numPlayers}"
    val roster = packet.lobbyPlayers.mapIndexed { idx, p ->
        "Player $idx → ${p.name} (#${p.carNumber}), Team=${p.teamId}, Nationality=${p.nationality}, " +
                "Platform=${p.platform}, AI=${p.aiControlled}, Ready=${p.readyStatus}"
    }.joinToString("\n")
...
// Car Damage (ID 10)
fun formatCarDamage23(packet: PacketCarDamageData): String {
    val playerIdx = packet.header.playerCarIndex.toInt()
...
fun formatSessionHistory23(packet: PacketSessionHistoryData): String {
...
// Tyre Sets (ID 12)
fun formatTyreSets23(packet: PacketTyreSetsData): String {
...
// MotionEx (ID ?)
fun formatMotionEx23(packet: PacketMotionExData): String {
    return """
        MotionEx →
