...
const val PACKET_ID_TIME_TRIAL: UByte = 14u
const val PACKET_ID_LAP_POSITIONS: UByte = 15u
...
object F1Parser25 {
    ...
    fun parse(buffer: ByteBuffer, header: PacketHeader): Any? {
        ...
        return when (header.packetId) {
            ...
            PACKET_ID_LAP_POSITIONS -> {
                val packet = parseLapPositions(buffer, header)
                Log.d(
                    "F1Parser",
                    "LapPositions → numLaps=${packet.numLaps}, lapStart=${packet.lapStart}"
                )
                packet
            }
            else -> {
    ...
    private fun parseParticipants(buffer: ByteBuffer, header: PacketHeader): PacketParticipantsData {
        ...
        repeat(numActiveCars.toInt()) {
            ...
            val numColours = buffer.get().toUByte()

            val liveryColours = List(4) {
                LiveryColour(
                    red = buffer.get().toUByte(),
                    green = buffer.get().toUByte(),
                    blue = buffer.get().toUByte()
                )
            }
    ...
    private fun parseFinalClassification(buffer: ByteBuffer, header: PacketHeader): PacketFinalClassificationData {
        ...
        repeat(numCars.toInt()) {
            ...
            val resultReason = buffer.get().toUByte()   // ✅ NEW in F1 25
            ...
            classificationData.add(
                FinalClassificationData(
                    ...
                    numPitStops,
                    resultStatus,
                    resultReason,              // ✅ added to constructor
                    bestLapTimeMs,
                    totalRaceTime,
                    penaltiesTime,
                    numPenalties,
    ...
    private fun parseLobbyInfo(buffer: ByteBuffer, header: PacketHeader): PacketLobbyInfoData {
        ...
        repeat(numPlayers.toInt()) {
            ...
            val nameBytes = ByteArray(32)   // ✅ F1 25 uses 32 bytes
            buffer.get(nameBytes)
            val name = nameBytes.toString(Charsets.UTF_8).trimEnd('\u0000')
    ...
    private fun parseCarDamage(buffer: ByteBuffer, header: PacketHeader): PacketCarDamageData {
        ...
        repeat(22) {
            ...
            val tyreBlisters = List(4) { buffer.get().toUByte() }   // ✅ NEW in F1 25
            ...
            val ersFault = buffer.get().toUByte()
            ...
            val engineBlown = buffer.get().toUByte()
            val engineSeized = buffer.get().toUByte()
            ...
            damages.add(
                CarDamageData(
                    ...
                    tyresDamage,
                    brakesDamage,
                    tyreBlisters,                 // ✅ added to constructor
                    frontLeftWingDamage,
    ...
    private fun parseMotionEx(buffer: ByteBuffer, header: PacketHeader): PacketMotionExData {
        ...
        val frontAeroHeight = buffer.float
        val rearAeroHeight = buffer.float
        val frontRollAngle = buffer.float
        val rearRollAngle = buffer.float
        val chassisYaw = buffer.float

        val chassisPitch = buffer.float                 // ✅ NEW in 25
        val wheelCamber = readFloatArray(4)             // ✅ NEW in 25
        val wheelCamberGain = readFloatArray(4)         // ✅ NEW in 25
    ...
    private fun parseLapPositions(buffer: ByteBuffer, header: PacketHeader): PacketLapPositionsData {
        buffer.order(ByteOrder.LITTLE_ENDIAN)

        val numLaps = buffer.get().toUByte()
        val lapStart = buffer.get().toUByte()

        // 50 laps × 22 cars
        val positionMatrix = List(50) {
            List(22) { buffer.get().toUByte() }
        }
        ...
        return PacketLapPositionsData(
            ...
            numLaps = numLaps,
            lapStart = lapStart,
            positionForVehicleIdx = positionMatrix
        )
    }
}
